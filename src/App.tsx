...
function App() {
  ...
  useEffect(() => {
    ...
    // Wrap fetch with a timeout so a hung request can never freeze the UI forever
    ...
    async function loadData() {
      ...
      try {
        ...
        // Run these sequentially (not Promise.all) — firing multiple
        // concurrent reads at the remote database connection at once
        // can cause one of them to hang indefinitely.
        const tradesRes = await fetchWithTimeout('/api/trades', { headers: authHeaders });
        const instRes = await fetchWithTimeout('/api/coach-instructions', { headers: authHeaders });
        const capRes = await fetchWithTimeout('/api/starting-capital', { headers: authHeaders });
        ...
        if (e instanceof DOMException && e.name === 'AbortError') {
          setLoadError('Server took too long to respond. Please retry.');
        } else {
      ...
      <div className="min-h-screen bg-white text-[#1C1C1E] flex flex-col items-center justify-center font-sans antialiased">
        <div className="flex flex-col items-center space-y-4">
          <div className="relative flex items-center justify-center">
            {/* Pulsing Outer Ring */}
            {!loadError && <div className="absolute w-16 h-16 rounded-full border-4 border-[#166534]/20 animate-ping" />}
            {/* Spinning Inner Arc */}
            {!loadError && <div className="w-16 h-16 rounded-full border-4 border-t-[#166534] border-r-transparent border-b-[#166534]/40 border-l-transparent animate-spin" />}
          ...
          <div className="text-center space-y-1 max-w-xs px-4">
            ...
            <p className={`text-xs font-bold text-[#5c5c5e] uppercase tracking-wider ${loadError ? '' : 'animate-pulse'}`}>
              {loadError ? loadError : 'Connecting to VPS Server...'}
            </p>
          </div>
          {loadError && (
