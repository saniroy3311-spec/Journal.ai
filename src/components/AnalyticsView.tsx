...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({ trades, onSelectTradeImage }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6 pb-24 md:pb-12 space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8 items-start">
        ...
        {/* Left: Navigation Menu - Vertical List Format (Non-sliding!) */}
        <div className="md:col-span-1 bg-[#FAFAF7] border border-[#D9D9D2] p-4 rounded-2xl premium-shadow space-y-2">
          <span className="text-[10px] font-extrabold text-[#5C5C5E] uppercase tracking-wider block mb-2 px-1">
            Analytics Menu
          </span>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer ${
                    isActive
                      ? 'bg-[#244230] text-white shadow-sm'
                      : 'text-[#5C5C5E] hover:text-[#1C1C1E] hover:bg-white border border-transparent hover:border-[#D9D9D2]/40'
                  }`}
                >
                  <Icon size={16} />
                  <span>{t.label}</span>
