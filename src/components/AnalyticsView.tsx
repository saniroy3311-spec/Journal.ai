  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-4 md:py-6 pb-24 md:pb-12 space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#D9D9D2] pb-5">
        ...
        <nav className="flex overflow-x-auto no-scrollbar bg-[#EAEAE2] p-1 rounded-xl relative self-start max-w-full gap-1">
          {subTabs.map((t) => {
            const Icon = t.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer shrink-0 ${
                  isActive ? 'bg-[#244230] text-white' : 'text-[#5C5C5E] hover:text-[#1C1C1E]'
                }`}
              >
                <Icon size={14} />
                <span>{t.label}</span>
      ...
      {closedTrades.length === 0 ? (
        <div className="bg-[#FAFAF7] border border-[#D9D9D2] border-dashed p-12 rounded-2xl text-center space-y-2">
          ...
          <p className="text-xs text-[#5C5C5E] font-medium">Please close at least one trade to activate advanced statistics.</p>
        </div>
      ) : (
              ...
              <div className="lg:col-span-2 bg-white border border-[#D9D9D2] p-3 sm:p-6 rounded-2xl premium-shadow space-y-4 sm:space-y-6">
                ...
                <div className="grid grid-cols-7 gap-1 sm:gap-2.5">
                  ...
                  {monthDays.map((day) => {
                    ...
                    if (hasTrades) {
                      if (netPnL > 5000) {
                        bgStyle = 'bg-[#D4E8DC] text-[#166534] border-[#5C8A6E] font-black';
                      } else if (netPnL > 0) {
                        bgStyle = 'bg-[#D4E8DC]/45 text-[#166534] border-[#D9D9D2] font-extrabold';
                      } else if (netPnL < -5000) {
                        bgStyle = 'bg-[#FADCDC] text-[#991B1B] border-[#B56B6B] font-black';
                      } else if (netPnL < 0) {
                        bgStyle = 'bg-[#FADCDC]/45 text-[#991B1B] border-[#D9D9D2] font-extrabold';
                      } else {
